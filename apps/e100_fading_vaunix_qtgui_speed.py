...
# Generated: Thu Feb 20 19:53:04 2014
##################################################
...
class e100_fading_vaunix_qtgui_speed(gr.top_block, Qt.QWidget):
	...
	def set_speed(self, speed):
		self.speed = speed
		self.set_fd(((1.0*self.speed/3.6)*self.carrierFreq)/3e8)
		self._speed_line_edit.setText(eng_notation.num_to_str(self.speed))
	...
	def set_carrierFreq(self, carrierFreq):
		...
		self.set_fd(((1.0*self.speed/3.6)*self.carrierFreq)/3e8)
		self._carrierFreq_line_edit.setText(eng_notation.num_to_str(self.carrierFreq))
	...
	def get_rfLeveldBm(self):
		return self.rfLeveldBm

	def set_rfLeveldBm(self, rfLeveldBm):
		self.rfLeveldBm = rfLeveldBm
		self._rfLeveldBm_counter.setValue(self.rfLeveldBm)
		self.set_atten(abs(self.rfLeveldBm+65))
	...
	def set_fd(self, fd):
		self.fd = fd
		self.set_fdTs(self.fd*(1.0/self.usrpRate))
		self.set_hhh(self.fd)

	def get_hhh(self):
		return self.hhh

	def set_hhh(self, hhh):
		self.hhh = hhh
		self._hhh_label.setText(eng_notation.num_to_str(self.hhh))
...
if __name__ == '__main__':
	...
	tb = e100_fading_vaunix_qtgui_speed()
	tb.start()
	tb.show()
	qapp.exec_()
	tb.stop()
